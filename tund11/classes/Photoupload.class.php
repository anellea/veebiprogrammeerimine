<?php
	class Photoupload {
		...
		public function addTextWatermark($text){
			//lisame teksti  
			$textColor = imagecolorallocatealpha($this->myImage, 255, 255, 255, 60); //0 ... 127 alpha- läbipaistvus,
		...
		public function saveOriginal($directory, $filename){
			...
			if (move_uploaded_file($this->tempFile, $target_file)) {
				$notice .= "Orginaalfail laeti üles! ";
			} else {
				$notice .= "Vabandust, orginaali üleslaadimisel tekkis tõrge! ";
			}
			return $notice;
		}
